...
public partial class FirewallPageView : UserControl
{
    // Keys intentionally keep their historical "dataSources." prefix so users'
    // saved column widths survive the move to the dedicated Firewall page.
    ...
    private FirewallPageViewModel? _attachedViewModel;
    private TopLevel? _topLevel;

    public FirewallPageView()
    {
        InitializeComponent();
        Loaded += OnViewLoaded;
    ...
    /// the page unbounded height, so without this cap the drawer's IP lists
    /// inflate the whole page's measured size — dragging the underlying grids
    ...
    private void AttachViewModel()
    {
        ...
        _attachedViewModel = DataContext as FirewallPageViewModel;
        if (_attachedViewModel != null)
        {
            _attachedViewModel.PropertyChanged += OnViewModelPropertyChanged;
        }
    ...
    {
        if (e.PropertyName == nameof(FirewallPageViewModel.FirewallSidePanelVisible))
        {
            UpdateDetailDrawerColumn();
        }
    }
    ...
    {
        if (DataContext is FirewallPageViewModel viewModel)
        {
    ...
    {
        if (DataContext is not FirewallPageViewModel viewModel)
        {
            return;
        }
    ...
    private static void PersistGridColumnWidths(DataGrid grid, string gridKey)
    {
        ...
        // All zeros means the grid has never been measured (page never opened) —
