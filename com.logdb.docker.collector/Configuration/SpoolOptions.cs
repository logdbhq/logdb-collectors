...
public class SpoolOptions
{
    ...
    /// oversized/slow flush can't blow the send timeout and force the whole batch to
    /// be re-sent. Keeps forward progress through a backlog (e.g. a log storm).
    /// </summary>
    public int SendChunkSize { get; set; } = 5_000;

    /// <summary>
    /// Cap on how many container-metric records the metrics spool retains while the
    /// exporter is unreachable. Oldest are dropped past this. Metrics are low-volume
    /// (containers × interval), so this buys many hours of outage coverage.
    /// </summary>
    public int MetricsMaxRecords { get; set; } = 200_000;
